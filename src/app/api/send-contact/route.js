...
async function sendViaNodemailer({ name, email, phone, subject, message, contactEmail, fromName, emailSubject }) {
  ...
  // bukan onboarding@resend.dev (karena Gmail akan menolaknya)
  const nodemailerFrom = process.env.SMTP_USER || "mail.sinarcerahsempurna@gmail.com";
...
export async function POST(request) {
  try {
    ...
    const contactEmail = (await getSetting("contact_email")) || process.env.CONTACT_EMAIL || "scs_construction@yahoo.com";
    const fromEmail = (await getSetting("contact_from_email")) || process.env.CONTACT_FROM_EMAIL || "onboarding@resend.dev";
